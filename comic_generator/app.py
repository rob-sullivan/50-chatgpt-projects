...
def slide_generator(text, prs):
    ...
    print("ChatGPT prompt DALL-e to: " + dalle_prompt)

    # I ask dalle to generate an image based on chatgpt prompt
    response = openai.Image.create(
        prompt=dalle_prompt + " Style: digital comicbook art",
        n=1,#I only want one image
        size="1024x1024")
    image_url = response['data'][0]['url']

    #I ask chatGPT to create bullet points
    prompt = f"Describe as if in comic format but display result in raw text with no special characters or headings: \n {text}"
    ...
    prompt = f"Create a title for a comic book panel from the following text and show result in raw format without special characters or headers: \n {text}"
    ppt = openai.ChatCompletion.create(
        model=model_engine,
        messages=[
            {"role": "user", "content": "I will ask you a question"},
...
app = tk.Tk()
app.title("Comic Book Generator")
